          ...
          <tbody>
            {documents.map(d => (
              <tr key={d.id}>
                ...
                <td><span className="role-tag" style={{ background: d.status === 'Delivered' ? '#d1fae5' : d.status === 'Rejected' ? '#fee2e2' : d.status === 'Processing' ? '#fef3c7' : '#e2e8f0', color: d.status === 'Delivered' ? '#065f46' : d.status === 'Rejected' ? '#991b1b' : d.status === 'Processing' ? '#92400e' : '#475569' }}>{d.status}</span></td>
                <td style={{ maxWidth: '200px', wordBreak: 'break-word' }}>{d.response_note ? <div style={{ fontStyle: 'italic', color: '#475569', fontSize: '13px' }}>"{d.response_note}"</div> : <span style={{ color: '#94a3b8' }}>-</span>}</td>
                {!isTeacher && (
                  <td>{d.status !== 'Delivered' && d.status !== 'Rejected' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                      {activeDocId === d.id ? (<>
                        ...
                        <div style={{ display: 'flex', gap: '5px' }}>
                          ...
                          <button onClick={() => handleStatusUpdate(d.id, 'Delivered')} style={{ flex: 1, background: '#10b981', color: 'white', border: 'none', padding: '5px', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{t('documents.markDelivered')}</button>
                        </div>
