...
function DashboardViceRector({ user, onLogout }) {
  ...
  const [view, setView] = useState(localStorage.getItem('vicerector_view') || 'overview');
  ...
  const handleViewChange = (newView) => {
    setView(newView);
    localStorage.setItem('vicerector_view', newView);
    if (newView === 'absences') {
      markAbsencesAsRead();
    }
  };
          ...
          loading ? (
            ...
            <div className="overview-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px' }}>
              <div className="card-academic" style={{ borderTop: '4px solid var(--p-indigo)' }}>
                <h3 style={{ fontSize: '12px', fontWeight: '800', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '16px' }}>Total Teachers (Univ)</h3>
              ...
              <div className="card-academic" style={{ borderTop: '4px solid #10b981' }}>
                <h3 style={{ fontSize: '12px', fontWeight: '800', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '16px' }}>Active Sessions (Univ)</h3>
